...
> Python（pandas）数据清洗 + RFM 用户分层 + Power BI 交互式看板，完整的电商数据分析项目。
...
> 再次说明：数据集无浏览/点击/加购日志，这只是订单状态漏斗，不是营销漏斗，不应表述为"转化率漏斗优化"。
...
- 复购阈值定义：1 次为首购基线，从第 2 次购买起计入复购，采用电商行业通用口径。
...
6. **高价值用户人均消费 436.90 雷亚尔，是全体人均（165.20）的 2.6 倍**；但因平台复购率仅 3%，其 GMV 占比只有 1.68%，经典“二八法则”并不成立——该平台以一次性购买为主，用户关系运营空间较大。
...
3. **复购率受数据窗口限制**：数据仅约 2 年，客户的长期复购潜力无法完整观测，分析时需注明。
